...
class TedgeBackend {
  ...
  emitter = {
    ...
    sendJobEnd: function (jobDefinition) {
      ...
      const followMode = job.mode || 'normal';
      if (job.jobName == 'configureTedge') {
        this.tedgeFileStore.upsertBackendConfiguration({
          status: 'INITIALIZED',
          deviceId: job.deviceId,
          cloudProvider: job.cloudProvider || 'c8y',
          c8yUrl: job.c8yUrl,
          tenantId: job.tenantId || null,
          awsUrl: job.awsUrl || null,
          azUrl: job.azUrl || null
        });
        this.tedgeFileStore.saveWebConfig({
          deviceId: job.deviceId,
          cloudProvider: job.cloudProvider || 'c8y',
          c8yUrl: job.c8yUrl,
          tenantId: job.tenantId || null,
  ...
  async getWebConfig(req, res) {
    this.tedgeFileStore.getWebConfig(req, res);
  }

  async saveWebConfigHttp(req, res) {
    this.tedgeFileStore.saveWebConfigHttp(req, res);
  }
  ...
  /** Builds a task that runs 'tedge <args>' in the correct mode. */
  _tedgeTask(mode, args, opts = {}) {
    const { cmd, prefix } = getTedgeBin(mode);
    return { cmd, args: [...prefix, ...args], ...opts };
  }

  /** Builds a task that runs 'tedgectl <args>' in the correct mode. */
  _tedgectlTask(mode, args, opts = {}) {
    const { cmd, prefix } = getTedgectlBin(mode);
    return { cmd, args: [...prefix, ...args], ...opts };
  }
  ...
  requestTedgeServiceStatus(job) {
    try {
      ...
      if (backendConfiguration.systemManager == 'openrc') {
        jobTasks = [
          this._tedgectlTask(mode, ['is_available'])
        ];
  ...
  requestTedgeConfiguration(job) {
    try {
      ...
      const mode = job.mode || 'normal';
      const jobTasks = [
        this._tedgeTask(mode, ['config', 'list'])
      ];
  ...
  resetTedge(job) {
    try {
      ...
      const mode = job.mode || 'normal';
      const jobTasks = [
        this._tedgeTask(mode, ['cert', 'remove']),
        this._tedgeTask(mode, ['disconnect', 'c8y']),
        this._tedgectlTask(mode, ['stop', 'mosquitto']),
        this._tedgectlTask(mode, ['stop', 'tedge-mapper-c8y']),
        this._tedgectlTask(mode, ['stop', 'tedge-agent']),
        {
          cmd: 'sudo',
  ...
  customCommand(job) {
    try {
      ...
      const mode = job.mode || 'normal';
      // If args start with 'tedge' or 'tedgectl', route through mode helper
      let jobTasks;
      if (job.args && job.args[0] === 'tedge') {
        jobTasks = [this._tedgeTask(mode, job.args.slice(1))];
      } else if (job.args && job.args[0] === 'tedgectl') {
        jobTasks = [this._tedgectlTask(mode, job.args.slice(1))];
      } else {
        jobTasks = [{ cmd: 'sudo', args: job.args }];
      }
  ...
  configureTedge(job) {
    try {
      const mode = job.mode || 'normal';
      const provider = job.cloudProvider || 'c8y';
      TedgeBackend.childLogger.info(
        `Starting configuration of edge: deviceId=${job.deviceId}, provider=${provider}`
      );

      let jobTasks = [
        this._tedgeTask(mode, ['cert', 'create', '--device-id', job.deviceId]),
        this._tedgeTask(mode, ['config', 'set', 'mqtt.bind.port', '1883']),
        this._tedgeTask(mode, ['config', 'set', 'mqtt.bind.address', '0.0.0.0'])
      ];

      if (provider === 'aws' && job.awsUrl) {
        jobTasks = [
          ...jobTasks,
          this._tedgeTask(mode, ['config', 'set', 'aws.url', job.awsUrl]),
          this._tedgectlTask(mode, ['restart', 'collectd'])
        ];
      } else if (provider === 'az' && job.azUrl) {
        jobTasks = [
          ...jobTasks,
          this._tedgeTask(mode, ['config', 'set', 'az.url', job.azUrl]),
          this._tedgectlTask(mode, ['restart', 'collectd'])
        ];
      } else {
        // default: c8y
        jobTasks = [
          ...jobTasks,
          this._tedgeTask(mode, ['config', 'set', 'c8y.url', job.c8yUrl]),
          this._tedgectlTask(mode, ['restart', 'collectd'])
        ];
      }
  ...
  stopTedge(job) {
    try {
      ...
      const mode = job.mode || 'normal';
      const jobTasks = [
        this._tedgeTask(mode, ['disconnect', 'c8y'], { continueOnError: true }),
        this._tedgectlTask(mode, ['stop', 'mosquitto'], { continueOnError: true }),
        this._tedgectlTask(mode, ['stop', 'tedge-mapper-c8y'], { continueOnError: true }),
        this._tedgectlTask(mode, ['stop', 'tedge-agent'], { continueOnError: true }),
        this._tedgectlTask(mode, ['stop', 'collectd'], { continueOnError: true }),
        this._tedgectlTask(mode, ['stop', 'tedge-mapper-collectd'], { continueOnError: true })
      ];
  ...
  startTedge(job) {
    try {
      ...
      const mode = job.mode || 'normal';
      const jobTasks = [
        this._tedgeTask(mode, ['connect', 'c8y'], { continueOnError: true }),
        this._tedgectlTask(mode, ['start', 'collectd'], { continueOnError: true }),
        this._tedgectlTask(mode, ['start', 'tedge-mapper-collectd'], { continueOnError: true })
      ];
