...
use crate::datalayer::{
    run_datalayer_loop, DatalayerEngine, DatalayerConfig, 
    MappingDirection, handle_mqtt_message 
};
...
impl TedgeDatalayerBridge {
    fn new(accept_invalid_certs: bool) -> Self {
        ...
        Self { mqtt_host: "localhost".to_string(), mqtt_port: 1883, http_client }
    }
    ...
    async fn setup_mqtt(&self, config: &DatalayerConfig) -> Result<mqtt::AsyncClient> {
        ...
        let create_opts = mqtt::CreateOptionsBuilder::new().server_uri(&broker).client_id("tedge-datalayer-bridge").finalize();
        let cli = mqtt::AsyncClient::new(create_opts)?;
        let conn_opts = mqtt::ConnectOptionsBuilder::new().keep_alive_interval(Duration::from_secs(20)).clean_session(true).automatic_reconnect(Duration::from_secs(1), Duration::from_secs(30)).finalize();
        cli.connect(conn_opts).await?;
        
        let mut topics = vec!["tedge/health/+".to_string()];
        for mapping in config.mappings.iter().filter(|m| m.direction == MappingDirection::TedgeToDatalayer && m.enabled) {
...
async fn main() -> Result<()> {
    ...
    let config_path: PathBuf = if is_snap {
        PathBuf::from(env::var("SNAP_DATA").unwrap_or_else(|_| "/var/snap/thin-edge-io/current".to_string())).join("datalayer-mappings.json")
    } else {
    ...
    let dl_handle = tokio::spawn(run_datalayer_loop(dl_engine, client_arc.clone(), shutdown.clone()));
